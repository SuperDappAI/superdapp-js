...
To switch providers, change your environment variables and restart the agent.`;
    ...
    agent.addCommand('/generate', async ({ roomId, message }) => {
      ...
      if (!prompt) {
        await agent.sendConnectionMessage(roomId, '💭 Please provide a prompt!\n\n**Usage:** `/generate Explain quantum physics`');
        return;
      }
      ...
      try {
        const aiProvider = process.env.AI_PROVIDER;
        const aiModel = process.env.AI_MODEL;
        console.log(`🤖 Generating text using ${aiProvider || 'unknown'} - ${aiModel || 'unknown'}`);
        ...
        await agent.sendConnectionMessage(roomId, `**Generated by ${aiProvider?.toUpperCase()} ${aiModel}:**\n\n${response}`);
      } catch (error: any) {
        console.error('Generation Error:', error);
        if (error.message?.includes('AI configuration')) {
          await agent.sendConnectionMessage(roomId, '⚠️ AI is not configured. Use `/status` to check configuration or `/help` for setup instructions.');
        } else {
    ...
    agent.addCommand('/compare', async ({ roomId, message }) => {
      const prompt = message.data?.split(' ').slice(1).join(' ');
      if (!prompt) {
        await agent.sendConnectionMessage(roomId, '🔍 Please provide a prompt to compare!\n\n**Usage:** `/compare Write a haiku about technology`');
        return;
      }
